#include <iostream>
#include <vector>


template<typename T>
class Node {
public:
    T key;
    Node<T>* left;
    Node<T>* right;

    Node(T data) {
        key = data;
        left = nullptr;
        right = nullptr;
    }
};


template<typename T>
class BST {
private:
    Node<T>* root;

    Node<T>* findRec(Node<T>* root, T key)
    {
        if (root == nullptr || root->key == key) {
            return root;
        }

        if (key < root->key) 
        {
            return findRec(root->left, key);
        }

        return findRec(root->right, key);
    }

    Node<T>* insertRec(Node<T>* root, T key) 
    {
        if (findRec(root, key))
        { 
            return root;
        }
        if (root == nullptr) 
        {
            return new Node<T>(key);
        }

        if (key < root->key) 
        {
            root->left = insertRec(root->left, key);
        } else if (key > root->key) 
        {
            root->right = insertRec(root->right, key);
        }

        return root;
    }

    void inorderRec(Node<T>* root) {
        if (root != nullptr) 
        {
            inorderRec(root->left);
            std::cout << root->key << "\n";
            inorderRec(root->right);
        }
    }

    int heightRec(Node<T>* root)
    {
        if (!root) return 0;
        int left_h = heightRec(root->left);
        int right_h = heightRec(root->right);
        return std::max(left_h, right_h) + 1;
    }



    int depthOfNode(Node<T>* root, T key)
    {
        if (root->key == key)
        {
            return 1;
        }

        if (key < root->key)
        {   
            return depthOfNode(root->left, key) + 1;
        }
        else {
            return depthOfNode(root->right, key) + 1;
        }

    }

    void deleteTree(Node<T>* root)
    {
        if (root)
        {
            deleteTree(root->left);
            deleteTree(root->right);
            delete root;
        }
    }

    const std::vector<T>& toArrayRec(Node<T>* root) 
    {
        static std::vector<int> vec;
        if (root)
        {
            toArrayRec(root->left);
            vec.push_back(root->key);
            toArrayRec(root->right);
        }
        return vec;
    }

    void listsRec(Node<T>* root)
    {
        if (root)
        {   
            listsRec(root->left);
            if (!root->left && !root->right)
            {
                std::cout << root->key << '\n';
            }
            listsRec(root->right);
        }
    }


    void forksDoubleRec(Node<T>* root)
    {
        if (root)
        {
            forksDoubleRec(root->left);
            if (root->left && root->right)
            {
                std::cout << root->key << '\n';
            }
            forksDoubleRec(root->right);
        }
    }


    void forksRec(Node<T>* root)
    {
        if (root)
        {
            forksRec(root->left);
            if (root->left && !root->right || root->right && !root->left)
            {
                std::cout << root->key << '\n';
            }
            forksRec(root->right);
        }  
    }



    bool isAVLRec(Node<T>* root)
    {
        if (root)
        {
            bool is_balanced_left = isAVLRec(root->left);
            bool is_balanced_right = isAVLRec(root->right);
            return abs(heightRec(root->left) - heightRec(root->right)) <= 1 
            && is_balanced_left && is_balanced_right;
        }
        return true;
    }

public:
    BST(): root(nullptr) {}

    int height()
    {
        return heightRec(root);
    }

    bool find(T key)
    {
        return findRec(root, key) != nullptr ? true : false;
    }

    void insert(T key)
    {
        root = insertRec(root, key);
    }

    void inorderTraversal()
    {
        inorderRec(root);
    }
    
    const std::vector<T>& toArray()
    {
        return toArrayRec(root);
    }

    int getDepth(T key)
    {
        return depthOfNode(root, key);
    }

    void forksDouble()
    {
        forksDoubleRec(root);
    }

    void forks()
    {
        forksRec(root);
    }

    void lists()
    {
        listsRec(root);
    }
    
    bool isAVL()
    {
        return isAVLRec(root);
    }
    
    ~BST()
    {
        deleteTree(root);
    }

};

int main() {
    BST<int> bst;
    int curr;
    std::cin >> curr;
    while(curr != 0)
    {
        bst.insert(curr);
        std::cin >> curr;        
    }
    std::cout << (bst.isAVL() ? "YES\n" : "NO\n");
}
